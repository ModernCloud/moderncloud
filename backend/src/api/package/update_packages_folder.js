...
module.exports = async (projectId) => {
    ...
    let packages = await Package.query()
        ...
        .where('project_id', projectId).groupBy(['name']);
    ...
    shelljs.exec(`npm install --no-fund --no-audit --prod --no-optional --ignore-scripts --no-bin-links --prefix "${projectPackages}"`, {async:true, silent: true});
};